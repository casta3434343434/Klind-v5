<script>
  ...
  const matches = $derived.by(() => {
    const places = (app.crags || []).filter(c => (c.tipo || 'palestra') === 'palestra');
...
<div class="field crag-field">
  ...
  <input type="text" autocomplete="off" value={query} oninput={onInput} onfocus={() => pickerOpen = query.trim().length > 0} placeholder="Cerca o scrivi un luogo...">
  {#if pickerOpen}
    <div class="crag-dropdown">
      ...
      {/each}
      {#if !matches.length}
        <div class="crag-option-empty">Nessun luogo trovato.</div>
      {/if}
    </div>
  {/if}
  {#if session.salvaLuogo}
    <div class="crag-add-form" style="margin-top:8px;">
      ...
      <p class="sub" style="font-size:12px;margin-top:6px;">Verrà aggiunto al database dei luoghi.</p>
    </div>
  {/if}
</div>
